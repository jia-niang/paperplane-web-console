...
import { GitCommit, GitCommonStatus, GitProject, GitRepo, GitStaff } from '@repo/db'
...
export class GitHelperService {
  ...
  async gitWeekly(projectId: string, staffId?: string) {
    ...
    const staffList = staffId ? staffs.filter((item: GitStaff) => String(item.id) === staffId) : staffs
