...
const wxBixProxyConfig = {
  httpsAgent: process.env.WXBIZ_PROXY_URL ? new HttpsProxyAgent(process.env.WXBIZ_PROXY_URL) : undefined,
}
...
export class WxBizService {
  ...
  async getAppMailAddress() {
    return this.client.get(`https://qyapi.weixin.qq.com/cgi-bin/exmail/app/get_email_alias`).then(res => res.data)
  }
